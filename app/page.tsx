...
export default function Home() {
  ...
  return (
    <main className="flex flex-col items-center justify-between bg-white text-slate-900 transition-colors duration-200 dark:bg-slate-950 dark:text-slate-50">
      <section className="w-full bg-slate-50 py-16 px-5 dark:bg-slate-900 md:px-10 lg:px-20">
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-12 lg:items-center">
          <div className="lg:col-span-7">
            ...
            <p className="mt-6 max-w-2xl text-base font-medium text-slate-600 dark:text-slate-300 sm:text-lg md:text-xl lg:text-2xl">
      ...
      {/* Statistics section */}
      <section className="w-full border border-slate-200 bg-neutral-300 dark:border-slate-800 dark:bg-slate-900 flex flex-col items-center justify-center p-6 shadow-lg sm:p-8">
        <div className="flex flex-col mb-5">
          <p className="flex items-start text-sm uppercase tracking-[0.3em] text-emerald-600 my-5">
            Statistics
          </p>
          <h1 className="font-bold text-3xl text-slate-900 dark:text-slate-50 md:text-[30px] text-center">
            Creating Better Access to Opportunities
          </h1>
        </div>

        <div className="flex flex-col lg:flex-row items-center justify-center gap-8 md:gap-12 md:mt-20 md:mb-20">
          {STATISTICS.map((card) => (
            <StatisticCard
              key={card.id}
              title={card.title}
      ...
      <section className="flex flex-col w-full bg-white dark:bg-slate-950 py-12 px-6 md:px-12">
        <h1 className="font-bold text-3xl text-slate-900 dark:text-slate-50 sm:text-4xl my-5 text-center">
          Featured Opportunities
        </h1>
